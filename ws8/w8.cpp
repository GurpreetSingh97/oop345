...
/*!
Merge function compares the values received from both the file and put those element in
a new list which have common product Code
*/
		...
		for (int j = 0; j < price.size(); j++){
			if (desc[i].code == price[j].code){
				try{
					...
					//validate function from element.h
					p->validate();
					priceList += p;
				}
				catch (...){
